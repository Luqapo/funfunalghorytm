...
function prepareData(dat) {
  ...
  dat
    ...
    .map(str => str.split(':'))
    .forEach((el) => {
      result[el[0]] = el[1].split(',');
    });
  Object.keys(result).forEach((key) => {
    result[key] = result[key].map(el => el.split(' '));
  });
  return result;
}

function mean(town, strng) {
  const result = prepareData(strng);
  if(!Object.keys(result).find(t => t === town)) return -1;
  return result[town].reduce((acc, d) => acc + +d[1], 0) / result[town].length;
}
function variance(town, strng) {
  const result = prepareData(strng);
  if(!Object.keys(result).find(t => t === town)) return -1;
  const avarge = mean(town, strng);
  return result[town].reduce((acc, d) => acc + ((d[1] - avarge) ** 2), 0) / result[town].length;
}
